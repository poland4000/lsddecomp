...
s32 GetStageChunkFromMood(StageChunk *out, MoodGraphPoint *mood) {
    u32 t2;
    s32 row;
    s32 col;

    for (t2 = 0; t2 < LENGTH_DIMENSIONS_TABLE; t2++) {
        MoodGraphPoint *p = STAGE_CHUNK_MOODS[t2];
        s32 rows = STAGE_GRID_DIMENSIONS[t2].rows;
        s32 columns = STAGE_GRID_DIMENSIONS[t2].columns;
